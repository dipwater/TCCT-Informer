...
class ConvLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        x_i = x.clone()    # Save residual

        if self.d == 1:
            # Process full sequence
            x_trans = x.permute(0, 2, 1)  # [B, D, L]
            x_p1 = self.downConv(x_trans)  # [B, D, L-2]
            x_p2 = self.pad1(x_i[:, 0:2, :].permute(0, 2, 1))  # [B, D, 2]
            x_p = torch.cat((x_p1, x_p2), dim=-1)  # [B, D, L]
        elif self.d == 2:
            # Split into even/odd
            x_p = torch.zeros(B, D, L, device=x.device, dtype=x.dtype)

            x1 = x[:, 0::2, :]   # even indices: [B, L1, D]
            x1_p1 = self.downConv(x1.permute(0, 2, 1))
            x1_p2 = self.pad1(x1[:, 0:2, :].permute(0, 2, 1))
            x1_p = torch.cat((x1_p1, x1_p2), dim=-1)  # [B, D, L1]

            x2 = x[:, 1::2, :]   # odd indices: [B, L2, D]
            x2_p1 = self.downConv(x2.permute(0, 2, 1))
            x2_p2 = self.pad1(x2[:, 0:2, :].permute(0, 2, 1))
            x2_p = torch.cat((x2_p1, x2_p2), dim=-1)  # [B, D, L2]
            ...
            for i in range(self.d):
                x_sub = x[:, i::self.d, :]  # [B, L_i, D]
                x_sub_p1 = self.downConv(x_sub.permute(0, 2, 1))      # [B, D, L_i - 2]
                x_sub_p2 = self.pad1(x_sub[:, 0:2, :].permute(0, 2, 1))  # [B, D, 2]
                x_sub_p = torch.cat((x_sub_p1, x_sub_p2), dim=-1)     # [B, D, L_i]

                # Compute target indices: i, i+d, i+2d, ...
                idxs = torch.arange(i, L, self.d, device=x.device)
                x_p[:, :, idxs] = x_sub_p

        # Apply normalization, activation, residual, pooling
        x_out = self.norm(x_p)
        x_out = self.dropout(self.activation(x_out))
        x_out = x_out + x_i.permute(0, 2, 1)  # Residual connection
        x_out = self.maxPool(x_out)
        x_out = x_out.transpose(1, 2)  # Back to [B, L_new, D]
        return x_out
